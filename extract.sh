...
# Image extraction
python2 extract.py --output "$DATA_DIR" --type image --path "$BAG" --topics "${IMG_TOPICS[@]}";

# IMU data extraction
python2 extract.py --output "$DATA_DIR" --type imu --path "$BAG" --topics "${IMU_TOPICS[@]}" --temp "${TEMP_TOPICS[@]}";
